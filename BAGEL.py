...
if sys.argv[1] == 'fc':
    ...
    for i in range(numColumns - 1):
        foldchange[normed.columns.values[i]] = log2(
            (normed.ix[:, normed.columns.values[i]]) / normed.ix[:, control_label])
    #
...
#-------------------------------------------#
    ...
    try:
        opts, args = getopt.getopt(sys.argv[2:], "hti:o:c:e:n:w:b", [
                                   "numcv=", "numiter=", "help", "bootstrapping"])
    except getopt.GetoptError:
        print(helptext('bf'))
        sys.exit(2)
    if len(opts) == 0:
    ...
    FC_THRESH = 2**(-1.1535 *
                    log(len(in1d(genes_array, nonEss))+13.324) + 0.7728)
    bf = {}
    boostedbf = {}
    ...
    for g in sorted(bf.keys()):
        ...
        if NETWORKBOOST == True:
            ...
            fout.write('{0:s}\t{1:4.3f}\t{2:4.3f}\t{3:4.3f}\t{4:4.3f}\t{5:d}\n'.format(g, float(
                boostedbf_mean), float(boostedbf_std), float(bf_mean[g]), float(bf_std[g]), num_obs))
        else:
            fout.write('{0:s}\t{1:4.3f}\t{2:4.3f}\t{3:d}\n'.format(
                g, float(bf_mean[g]), float(bf_std[g]), num_obs))
    fout.close()
